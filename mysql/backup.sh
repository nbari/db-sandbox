...
# ----------------------------------------------------------------------------
hash mysqldump > /dev/null 2>&1 && ${MYSQLDUMP_PATH} -h ${HOST} --user=${USER} --password=${PASS} --events --compress --hex-blob --opt --skip-comments ${DB_NAME} | bzip2 > /${BACKUP_PATH}/${DB_NAME}/${DATE}.sql.bz2 || {
